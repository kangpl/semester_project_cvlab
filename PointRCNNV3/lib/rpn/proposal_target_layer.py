...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if cfg.RCNN.USE_INTENSITY:
            pts_extra_input_list = [input_dict['rpn_intensity'].unsqueeze(dim=2),  #(B,N,1)
                                    input_dict['seg_mask'].unsqueeze(dim=2)]
        else:
            pts_extra_input_list = [input_dict['seg_mask'].unsqueeze(dim=2)] #(B,N,1)
        ...
        # point cloud pooling
        pts_feature = torch.cat((pts_extra_input, rpn_features), dim=2) #(B,N,2+C)
