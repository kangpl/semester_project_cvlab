...
class PointRCNN(nn.Module):
    def __init__(self, num_classes, use_xyz=True, mode='TRAIN'):
        ...
        self.merge_down = pt_utils.SharedMLP([feature_channel * 2, feature_channel], bn=cfg.RPN.USE_BN)
    ...
    def forward(self, input_data):
        if cfg.RPN.ENABLED:
            ...
            if cfg.RCNN.ENABLED:
                ...
                backbone_features = torch.cat((backbone_features.unsqueeze(dim=3), img_features.unsqueeze(dim=3)), dim=1)
