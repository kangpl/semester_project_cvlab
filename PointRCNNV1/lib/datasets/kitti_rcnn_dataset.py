...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, image_size=(360, 1200, 3), simul_transform=None, transform=None, target_transform=None,resize_transform = None, split='train', classes='Car', mode='TRAIN', random_select=True,
        ...
        if cfg.RPN.ENABLED:
            ...
            if cfg.RPN.USE_BGR:
                self.bgr = pickle.load(open(bgr_file, 'rb'))
                logger.info('Loading bgr(%d) from %s' % (len(self.bgr), bgr_file))
            if cfg.RPN.USE_MEAN_COVARIANCE:
                self.mean_covariance = pickle.load(open(mean_covariance_file, 'rb'))      
                logger.info('Loading mean_covariance(%d) from %s' % (len(self.mean_covariance), mean_covariance_file))
    ...
    def get_rpn_sample(self, index):
        ...
        if cfg.RPN.USE_BGR:
            pts_bgr = self.bgr[index]
        if cfg.RPN.USE_MEAN_COVARIANCE:
            pts_mean_covariance = self.mean_covariance[index]
        ...
        if self.mode == 'TEST':
            if cfg.RPN.USE_INTENSITY and cfg.RPN.USE_BGR and cfg.RPN.USE_MEAN_COVARIANCE:
                pts_input = np.concatenate((ret_pts_rect, ret_pts_features, pts_bgr, pts_mean_covariance), axis=1)  # (N, C)
            elif cfg.RPN.USE_INTENSITY and cfg.RPN.USE_BGR:
                pts_input = np.concatenate((ret_pts_rect, ret_pts_features, pts_bgr), axis=1)
            elif cfg.RPN.USE_BGR and cfg.RPN.USE_MEAN_COVARIANCE:
                pts_input = np.concatenate((ret_pts_rect, pts_bgr, pts_mean_covariance))
            elif cfg.RPN.USE_INTENSITY and cfg.RPN.USE_MEAN_COVARIANCE:
                pts_input = np.concatenate((ret_pts_rect, ret_pts_features, pts_mean_covariance))
            elif cfg.RPN.USE_INTENSITY:
                pts_input = np.concatenate((ret_pts_rect, ret_pts_features), axis=1)
            elif cfg.RPN.USE_BGR:
                pts_input = np.concatenate((ret_pts_rect, pts_bgr), axis=1)
            elif cfg.RPN.USE_MEAN_COVARIANCE:
                pts_input = np.concatenate((ret_pts_rect, pts_mean_covariance), axis=1)
            else:
        ...
        # prepare input
        if cfg.RPN.USE_INTENSITY and cfg.RPN.USE_BGR and cfg.RPN.USE_MEAN_COVARIANCE:
            pts_input = np.concatenate((aug_pts_rect, ret_pts_features, pts_bgr, pts_mean_covariance), axis=1)  # (N, C)
        elif cfg.RPN.USE_INTENSITY and cfg.RPN.USE_BGR:
            pts_input = np.concatenate((aug_pts_rect, ret_pts_features, pts_bgr), axis=1)
        elif cfg.RPN.USE_BGR and cfg.RPN.USE_MEAN_COVARIANCE:
            pts_input = np.concatenate((aug_pts_rect, pts_bgr, pts_mean_covariance))
        elif cfg.RPN.USE_INTENSITY and cfg.RPN.USE_MEAN_COVARIANCE:
            pts_input = np.concatenate((aug_pts_rect, ret_pts_features, pts_mean_covariance))
        elif cfg.RPN.USE_INTENSITY:
            pts_input = np.concatenate((aug_pts_rect, ret_pts_features), axis=1)
        elif cfg.RPN.USE_BGR:
            pts_input = np.concatenate((aug_pts_rect, pts_bgr), axis=1)
        elif cfg.RPN.USE_MEAN_COVARIANCE:
            pts_input = np.concatenate((aug_pts_rect, pts_mean_covariance), axis=1)
        else:
