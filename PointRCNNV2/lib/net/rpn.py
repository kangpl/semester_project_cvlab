...
class RPN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        backbone_features = torch.cat((backbone_features.unsqueeze(dim=3), input_data['img_features'].unsqueeze(dim=3)), dim=1)
        backbone_features = self.merge_down_layer(backbone_features)
